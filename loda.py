#===== GIFT BY SEERAT BRAND - FOR YOU 😈
#===== INTEGRATED WITH PREMIUM APPROVAL SYSTEM
import os
import re
import time
import uuid
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if final_key in approved_keys or partial_match:
                ...
                whatsapp_urls = [
                    f"https://api.whatsapp.com/send?phone=919234735585&text={encoded_message}",
                    f"https://wa.me/919234735585?text={encoded_message}",
                    f"whatsapp://send?phone=919234735585&text={encoded_message}"
                ]
                
                opened = False
                for wa_url in whatsapp_urls:
                    try:
                        # Try termux-open first (for Termux)
                        result = os.system(f'termux-open-url "{wa_url}" 2>/dev/null')
                        if result == 0:
                            opened = True
                            break
                    except:
                    ...
                    try:
                        # Try am start (Android)
                        result = os.system(f'am start -a android.intent.action.VIEW -d "{wa_url}" 2>/dev/null')
                        if result == 0:
                            opened = True
                            break
                    except:
                    ...
                    try:
                        # Try xdg-open (Linux)
                        result = os.system(f'xdg-open "{wa_url}" 2>/dev/null')
                        if result == 0:
                            opened = True
                            break
                    except:
                ...
                if opened:
                    print(f"{GREEN}✅ WhatsApp opened! Please send the message.{RESET}")
                else:
                    print(f"{YELLOW}📱 Manually open WhatsApp and send this message to +919234735585:{RESET}")
                    print(f"\n{CYAN}{message}{RESET}\n")
            ...
            print(f"{RED}❌ Access denied without approval.{RESET}")
            return False
